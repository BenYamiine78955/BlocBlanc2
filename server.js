...
app.get('/api/vehicule/count', (req, _res, next) => {
  req.requiredroles = ["admin"];
  next();
}, verifyTokenAndRole, (req, res) => {
  ...
  db.query(sql, (err, results) => {
    ...
    console.log('Nombre vehicles:', results);
...
app.get('/api/vehicules', (req, _res, next) => {
  ...
  console.log('Fetching vehicles for user ID:', req.user.id);
  ...
  db.query(sql, (err, results) => {
    ...
    console.log('Fetched vehicles:', results);
...
app.post('/api/vehicules/add', (req, res) => {
  ...
  verifyTokenAndRole(req, res, () => {
    ...
    db.query(sql, [marque, modele, annee, client_id], (err, result) => {
      if (err) {
        console.error('Erreur Insertion vehicule:', err);
        return res.status(500).send('Server error');
      }
      res.status(201).send('Vehicule ajouté');
    });
  });
});

// Route PUT pour modifier un véhicule
app.put('/api/vehicules/edit/:id', (req, res) => {
  req.requiredroles = ["admin"];
  verifyTokenAndRole(req, res, () => {
    const { id } = req.params;
    const { marque, modele, annee, client_id } = req.body;
